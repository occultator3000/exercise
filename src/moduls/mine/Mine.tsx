...
import icon_no_favorite from '../../assets/icon_no_favorite.webp';

const EMPTY_CONFIG = [
  { icon: icon_no_note, tips: '还没有笔记，快去创作吧~' },
  { icon: icon_no_collection, tips: '还没有收藏，快去收藏喜欢的笔记吧~' },
  { icon: icon_no_favorite, tips: '还没有赞过，快去点赞喜欢的笔记吧~' },
];
...
export default observer(() => {
  ...
  const renderList = () => {
    ...
    if (!currentList?.length) {
      const { icon, tips } = EMPTY_CONFIG[tabIndex];
      return <Empty icon={icon} tips={tips} />;
    }
    return (
        ...
        {currentList.map((item, index) => {
          return (
            ...
            >
              ...
              <View style={styles.noteNameLayout}>
                <Image
                  style={styles.noteAvatarImg}
                  source={{ uri: item.avatarUrl }}
                />
                <Text style={styles.noteNameTxt}>{item.userName}</Text>
                <Heart
