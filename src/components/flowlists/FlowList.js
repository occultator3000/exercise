...
export default class FlowList extends React.PureComponent {
  ...
  _computePositions = byRender => {
    ...
    const { data = [], numColumns = 2 } = this.props;
    ...
    let rowData = [];
    let rowOffsetTop = 0;
    originItems.forEach((itemData, dataIndex) => {
      ...
      const bottomItem = rowData.sort(
        (a, b) =>
          (b.height || 0) + b.offsetTop - ((a.height || 0) + a.offsetTop),
      )[0];
      const height =
        bottomItem.offsetTop - rowOffsetTop + (bottomItem.height || 0);
      dataSource[rowIndex] = {
        rowIndex,
        rowData,
        height,
      ...
      if (dataIndex === itemData.length - 1) {
  ...
  _updateIfNeeded = () => {
    ...
    if (
      !itemHeights.some(o => o === null) &&
      !itemDidUpdates.some(
        (o, index) =>
          o === null && this._itemRefs[Math.floor(index / numColumns)],
      ) &&
      itemDidUpdates.some(o => o === 2)
    ) {
  ...
  scrollToOffset = ({ animated, offset }) =>
    this._listRef.scrollToOffset({ animated, offset });
  ...
  render() {
    ...
    return (
      <FlatList
        ...
        onEndReached={info => {
          if (
            !JSON.parse(JSON.stringify(this._itemHeights)).some(o => o === null)
          ) {
          ...
          return (
            <FlowListItem
              ...
              onItemDidUpdate={flag => {
                const { rowData } = item;
                const itemDidUpdates = JSON.parse(
                  JSON.stringify(this._itemDidUpdates),
                );
                rowData.forEach(rowItemData => {
                  const { rowItemIndex } = rowItemData;
                  if (itemDidUpdates[rowItemIndex] === null) {
                    this._itemDidUpdates[rowItemIndex] = flag;
                  }
                });
                this._updateIfNeeded();
              }}
              onItemHeightChange={(height, itemIndex) => {
                const preHeight = this._itemHeights[itemIndex];
                if (this._itemHeights[itemIndex] !== height) {
                  const preAllLoaded = !JSON.parse(
                    JSON.stringify(this._itemHeights),
                  ).some(o => o === null);
                  this._itemHeights[itemIndex] = height;
                  const allLoaded = !JSON.parse(
                    JSON.stringify(this._itemHeights),
                  ).some(o => o === null);
                  if (allLoaded) {
                    if (preAllLoaded) {
                      this._itemDidUpdates[itemIndex] = 2;
                      if (
                        preHeight &&
                        height &&
                        Math.abs(preHeight - height) >= 0.01
                      ) {
                        this._updateIfNeeded();
                      }
                    } else {
